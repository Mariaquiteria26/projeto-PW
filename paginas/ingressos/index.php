...
$sql = "SELECT ingressos.*, 
               sessoes.data, 
               sessoes.horario, 
               sessoes.sala,
               filmes.nome AS nome_filme
        FROM ingressos
        INNER JOIN sessoes ON ingressos.sessao_id = sessoes.id
...
<head>
    ...
    <title>Ingressos - Cinema Control</title>
    ...
    <aside class="w-60 min-h-screen bg-[#050505] border-r border-zinc-900 flex flex-col justify-between p-5">
        <div>
            ...
            <nav class="space-y-3">
                ...
                <a href="../ingressos/index.php" class="flex items-center gap-3 bg-orange-500/10 text-orange-400 px-4 py-3 rounded-2xl border border-orange-500/20 shadow-[0_0_20px_rgba(249,115,22,0.12)]">
                    <i data-lucide="ticket" class="w-5 h-5"></i>
                    <span class="text-sm font-medium">Ingressos</span>
                </a>
            </nav>
        </div>
    ...
    <main class="flex-1 p-8">
        ...
        <div class="flex items-center justify-between mb-8">
            <div class="flex items-center gap-5">
                <div class="w-16 h-16 rounded-2xl bg-orange-500/10 border border-orange-500/30 flex items-center justify-center shadow-[0_0_25px_rgba(249,115,22,0.18)]">
                    <i data-lucide="ticket" class="w-9 h-9 text-orange-400"></i>
                </div>

                <div>
                    <h1 class="text-4xl font-bold">Ingressos</h1>
                    <p class="text-zinc-400 mt-1">
                        Gerencie os ingressos cadastrados no Cinema Control
                    </p>
                </div>
            </div>
        ...
        <section class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-7">
            <div class="bg-[#101014] border border-zinc-900 rounded-3xl p-6">
                ...
                <p class="text-zinc-400 text-sm">Total de ingressos</p>
            ...
            <table class="w-full">
                ...
                <thead class="bg-zinc-900/80">
                    <tr>
                        ...
                        <th class="text-left p-5 text-zinc-400">Cliente</th>
                        ...
                        <th class="text-left p-5 text-zinc-400">Quantidade</th>
                    ...
                    <?php foreach($ingressos as $ingresso): ?>
                        ...
                        <tr class="border-t border-zinc-900 hover:bg-zinc-900/50 transition">
                            ...
                            <td class="p-5 text-zinc-300">
                                <?php echo $ingresso['id']; ?>
                            </td>

                            <td class="p-5 font-medium">
                                <div class="flex items-center gap-3">
                                    <span class="w-2 h-2 rounded-full bg-orange-500"></span>
                                    <?php echo htmlspecialchars($ingresso['cliente']); ?>
                                </div>
                            </td>

                            <td class="p-5 text-zinc-300">
                                <?php echo htmlspecialchars($ingresso['nome_filme']); ?>
                            </td>

                            <td class="p-5 text-zinc-300">
                                <?php echo date('d/m/Y', strtotime($ingresso['data'])); ?>
                            </td>

                            <td class="p-5">
                                <span class="bg-orange-500/15 text-orange-400 border border-orange-500/30 px-4 py-2 rounded-xl text-sm font-semibold">
                                    <?php echo date('H:i', strtotime($ingresso['horario'])); ?>
                                </span>
                            </td>

                            <td class="p-5 text-zinc-300">
                                <?php echo htmlspecialchars($ingresso['sala']); ?>
                            </td>

                            <td class="p-5">
                                <span class="bg-zinc-900 text-zinc-300 border border-zinc-800 px-4 py-2 rounded-xl text-sm">
                                    <?php echo $ingresso['quantidade']; ?>
                                </span>
                            </td>
                            ...
                            <td class="p-5">
                                <div class="flex gap-3">
                                    ...
                                    <a href="editar.php?id=<?php echo $ingresso['id']; ?>" class="w-10 h-10 flex items-center justify-center rounded-xl border border-orange-500/30 text-orange-400 hover:bg-orange-500/10 transition">
                                        <i data-lucide="pencil" class="w-5 h-5"></i>
                                    </a>

                                    <a href="delete.php?id=<?php echo $ingresso['id']; ?>" onclick="return confirm('Tem certeza que deseja excluir?')" class="w-10 h-10 flex items-center justify-center rounded-xl border border-red-500/30 text-red-400 hover:bg-red-500/10 transition">
